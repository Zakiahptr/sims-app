...
<x-breadcrumbs>
    ...
    <x-slot name="menu_1">Edit Produk</x-slot>
</x-breadcrumbs>

<form id="productFormEdit" action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('patch')
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            ...
            <input id="purchase_price" type="text" class="form-control @error('purchase_price') is-invalid @enderror"
                   name="purchase_price" value="{{ number_format(old('purchase_price', $product->purchase_price), 0, ',', '.') }}"
        ...
        <div class="col-md-4">
            ...
            <input id="selling_price" type="text" class="form-control @error('selling_price') is-invalid @enderror"
                   name="selling_price" value="{{ number_format(old('selling_price', $product->selling_price), 0, ',', '.') }}"
        ...
        <div class="col-md-4">
            ...
            <input id="stock" type="number" class="form-control @error('stock') is-invalid @enderror"
                   name="stock" value="{{ old('stock', $product->stock) }}" placeholder="Masukkan stok">
    ...
    <div class="row mb-3">
        <div class="col-md-12">
            ...
            <div id="image-upload-container" class="image-upload-box text-center">
                ...
                <div id="preview-container" class="{{ $product->image ? '' : 'd-none' }}">
                    <img id="preview-image" src="{{ asset('storage/' . $product->image) }}" alt="Product Image">
                    <p id="file-name">{{ $product->image ? basename($product->image) : '' }}</p>
                </div>
            </div>
            @error('image')
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Jika ada gambar lama, tampilkan saat pertama kali halaman dimuat
        if (previewImage.src && previewImage.src !== window.location.href) {
            previewContainer.classList.remove("d-none");
            previewIcon.classList.add("d-none");
            uploadText.classList.add("d-none");
            fileName.textContent = previewImage.src.split('/').pop();
        }

        // Event saat klik box upload
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Format angka ke ribuan dengan titik (10.000)
        ...
        function calculateSellingPrice() {
            let purchaseValue = purchaseInput.value.replace(/\D/g, ""); // Ambil angka tanpa titik
            if (purchaseValue) {
        ...
        formatPrice(purchaseInput);
        formatPrice(sellingInput);
        ...
        document.getElementById("productFormEdit").addEventListener("submit", function () {
